...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: 'github',   url: 'https://github.com/bassem669',                              color: '#333',    label: 'GitHub' },
    { icon: 'linkedin', url: 'https://www.linkedin.com/in/bassem-mathlouthi-3abab6362/', color: '#0077b5', label: 'LinkedIn' },
    { icon: 'facebook', url: 'https://www.facebook.com/bassam.mathlouthi.1',             color: '#1877f2', label: 'Facebook' }
  ];
            ...
            <Col className="mb-3 mb-md-0">
              ...
              <div className="d-flex flex-column gap-1 gap-md-2">
                {quickLinks.map((link, index) => (
                  ...
                  >
                    <i className="fas fa-chevron-right me-2" style={{ fontSize: isMobile ? '0.6rem' : '0.7rem' }}></i>
                    {link.label}
                  </a>
                ))}
              </div>
              ...
              <div className="d-flex gap-2 gap-md-3">
                {socialLinks.map((social, index) => (
                  <a
                    ...
                    onTouchStart={(e) => { if (isMobile) e.currentTarget.style.background = social.color; }}
            ...
            <Col md={6} className={isMobile ? 'text-center' : 'text-center text-md-end'}>
              <div className={`d-flex flex-column ${isMobile ? 'align-items-center' : 'flex-md-row align-items-center justify-content-md-end'} gap-1 gap-md-2`}>
                <a
                  ...
                  style={{ fontSize: isMobile ? '0.8rem' : '0.9rem', wordBreak: 'break-word' }}
                  onMouseEnter={(e) => { if (!isMobile) { e.currentTarget.style.opacity = '1'; e.currentTarget.style.color = '#3b82f6'; } }}
                  onMouseLeave={(e) => { if (!isMobile) { e.currentTarget.style.opacity = '0.75'; e.currentTarget.style.color = ''; } }}
                  onTouchStart={(e) => { if (isMobile) e.currentTarget.style.color = '#3b82f6'; }}
                  onTouchEnd={(e)   => { if (isMobile) e.currentTarget.style.color = ''; }}
                >
                  <i className="fas fa-envelope me-2"></i>
                  {isMobile ? 'Email' : 'bassemmathlouthi05@gmail.com'}
                </a>

                {!isMobile && <span className="text-light opacity-50 mx-2">•</span>}

                <a
                  href="tel:+21626557704"
                  className="text-light opacity-75 text-decoration-none d-flex align-items-center justify-content-center"
                  style={{ fontSize: isMobile ? '0.8rem' : '0.9rem' }}
                  onMouseEnter={(e) => { if (!isMobile) { e.currentTarget.style.opacity = '1'; e.currentTarget.style.color = '#10b981'; } }}
                  onMouseLeave={(e) => { if (!isMobile) { e.currentTarget.style.opacity = '0.75'; e.currentTarget.style.color = ''; } }}
                  onTouchStart={(e) => { if (isMobile) e.currentTarget.style.color = '#10b981'; }}
                  onTouchEnd={(e)   => { if (isMobile) e.currentTarget.style.color = ''; }}
                >
